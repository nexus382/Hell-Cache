-- pages/page30.lua
-- Test Page 30: Bitmask API (setGroupMask, getGroupMask, setCollidesWithGroupsMask, getCollidesWithGroupsMask)
...
-- Collision group constants (bitmask style)
local GROUP_PLAYER = 0x00000001        -- Group 1 (bit 0)
local GROUP_ENEMY = 0x00000002         -- Group 2 (bit 1)
local GROUP_PLAYER_BULLET = 0x00000004 -- Group 3 (bit 2)
local GROUP_ENEMY_BULLET = 0x00000008  -- Group 4 (bit 3)
...
--- @brief Load sprites and set up collision groups using bitmask API
local function loadSprites()
    ...
    if not player_handle or not enemy_handle or not player_bullet_handle or not enemy_bullet_handle then
        vmupro.system.log(vmupro.system.LOG_ERROR, "Page30", "Failed to load sprites")
        load_error = true
        return
    end
    ...
    -- Setup collision groups using BITMASK API
    -- Player: belongs to group 1, collides with enemy (2) and enemy bullets (4)
    vmupro.sprite.setGroupMask(player_handle, GROUP_PLAYER)
    vmupro.sprite.setCollidesWithGroupsMask(player_handle, GROUP_ENEMY | GROUP_ENEMY_BULLET)

    -- Enemy: belongs to group 2, collides with player (1) and player bullets (3)
    vmupro.sprite.setGroupMask(enemy_handle, GROUP_ENEMY)
    vmupro.sprite.setCollidesWithGroupsMask(enemy_handle, GROUP_PLAYER | GROUP_PLAYER_BULLET)

    -- Player bullet: belongs to group 3, collides with enemy (2) only
    vmupro.sprite.setGroupMask(player_bullet_handle, GROUP_PLAYER_BULLET)
    vmupro.sprite.setCollidesWithGroupsMask(player_bullet_handle, GROUP_ENEMY)

    -- Enemy bullet: belongs to group 4, collides with player (1) only
    vmupro.sprite.setGroupMask(enemy_bullet_handle, GROUP_ENEMY_BULLET)
    vmupro.sprite.setCollidesWithGroupsMask(enemy_bullet_handle, GROUP_PLAYER)
    ...
    sprites_loaded = true
    vmupro.system.log(vmupro.system.LOG_INFO, "Page30", "Sprites loaded with bitmask collision groups")
end

--- @brief Check if sprite A should collide with sprite B using bitmask API (efficient)
...
function Page30.exit()
    ...
    vmupro.system.log(vmupro.system.LOG_INFO, "Page30", "Collision groups cleared, sprites freed")
end

--- @brief Render Page 30: Bitmask API Demo
function Page30.render(drawPageCounter)
    ...
    vmupro.graphics.drawText("Bitmask API", 10, 10, vmupro.graphics.WHITE, vmupro.graphics.BLACK)
    ...
    elseif player_handle and enemy_handle and player_bullet_handle and enemy_bullet_handle then
        ...
        -- Check collisions with group filtering (using bitmask API)
        ...
        local y_info = 155
        vmupro.graphics.drawText("P:0x01 E:0x02 PB:0x04 EB:0x08", 10, y_info, vmupro.graphics.GREY, vmupro.graphics.BLACK)
        y_info = y_info + 15
        vmupro.graphics.drawText("PB mask & E collides", 10, y_info, vmupro.graphics.GREY, vmupro.graphics.BLACK)
        y_info = y_info + 15
        vmupro.graphics.drawText("EB mask & P collides", 10, y_info, vmupro.graphics.GREY, vmupro.graphics.BLACK)
    ...
    -- Description at top
    vmupro.graphics.drawText("Efficient bitmask filtering", 10, 40, vmupro.graphics.GREY, vmupro.graphics.BLACK)
